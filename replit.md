...
-   **Financial Goals System**: User-defined goals for debt payoff, savings, and emergency fund tracking. Features include:
    ...
    - **Smart Debt Payoff Tracking (Dec 2025)**: Debt payoff goals require selecting a linked credit card. The `startingAmount` captures the card balance when the goal is created. Progress is calculated as `(startingAmount - currentBalance) / startingAmount`. The UI shows: "X paid of Y" (amount paid vs original), "Z remaining" (current balance), "+A added" (new debt since goal creation, shown in yellow if any), and "since [date]" (goal creation date). This provides full visibility into both payoff progress AND new spending accountability.
