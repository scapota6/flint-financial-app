...
- **Subscription System**: Three-tier model (Free, Basic, Pro) integrated with Whop.com for payment processing and feature gating, with calendar-aware next billing date calculations. **Checkout Flow**: Embedded checkout modal using `@whop/checkout/react` React package with `WhopCheckoutEmbed` component. Backend creates checkout session via Whop API (POST https://api.whop.com/api/v2/checkout_sessions) and returns sessionId. Modal renders Whop's checkout iframe with dark theme, email prefill, sessionId, and loading fallback. Component uses `onComplete` callback to capture planId and receiptId after successful payment. Server-side endpoint `POST /api/whop/activate-subscription` (requires authentication + CSRF validation) performs comprehensive 7-step payment verification via Whop REST API before granting subscription access: validates payment completion, user email match, refund status, and plan ID match. Backend uses plan IDs (plan_n0ViyXRIGl5q4, plan_eK9OYHEBv6qac, plan_McZjMF8v2N655, plan_1ZXWf1eqiwjcT) configured in `server/lib/whop-config.ts`. After activation, user redirects to dashboard. Webhooks handle subscription lifecycle events, user creation, and email delivery. Free: 4 connections, money in/out flow, dashboard, transaction history. Basic: unlimited connections, all Free features + recurring subscriptions, credit card management, stock charts (coming soon). Pro: all Basic features + trading & transfers (coming soon).
...
- **SnapTrade Integration**: Stores user credentials and brokerage authorizations in `snaptrade_users` table, consolidated registration, server-side auto-sync, normalized account display names, auto-recovery for orphaned accounts. **Trading Capabilities**: Brokerage capability detection system identifies trading-enabled vs read-only accounts. Trading-supported brokerages: Coinbase, Alpaca, Robinhood, E*Trade, Schwab, TradeStation, Tradier, Webull, Interactive Brokers, TD Ameritrade, Tastytrade. Read-only brokerages: Chase, Fidelity, Vanguard. All account endpoints include `tradingEnabled` and `capabilities` fields. **Real-time Market Data**: Live quotes via `/api/quotes/:symbol` endpoint pulling from SnapTrade with 1-second polling intervals. All trading components updated for live data feeds. **Mobile OAuth Support**: `/api/connections/snaptrade/register` endpoint accepts custom `redirectUri` from request body for mobile deep links (e.g., `flintapp://snaptrade-callback`), enabling OAuth flows on iOS/React Native apps. Returns both `url` and `redirectUrl` fields for backward compatibility with web clients. **Mobile OAuth Callback**: `POST /api/snaptrade/callback` and `POST /api/snaptrade/sync` endpoints support dual-mode authentication via `requireAuth` middleware - accept both Bearer tokens (mobile apps via Authorization header) and httpOnly cookies (web browsers). Endpoints accept `authorizationId` or `connection_id` from request body to complete OAuth flow, sync only the specific brokerage connection to database, return 400 if ID missing or 404 if connection not found. **Mobile Portfolio API**: `/api/portfolio-holdings` endpoint includes mobile-friendly field aliases (`value`, `shares`, `gainLoss`, `gainLossPercent`) alongside existing web app fields for iOS/React Native compatibility. **Webhook Integration**: Real-time webhook system at `/api/webhooks/snaptrade` handles all official SnapTrade events (CONNECTION_BROKEN, CONNECTION_FIXED, CONNECTION_ADDED, CONNECTION_DELETED, CONNECTION_UPDATED, NEW_ACCOUNT_AVAILABLE, ACCOUNT_HOLDINGS_UPDATED, ACCOUNT_TRANSACTIONS_UPDATED, TRADES_PLACED, etc.) per https://docs.snaptrade.com/docs/webhooks. Secret verified from request body `webhookSecret` field. **CONNECTION_BROKEN/CONNECTION_DELETED webhooks completely DELETE connections and all child records** (accounts, balances, positions, orders, activities, option_holdings) via transactional cascading deletion using batched `inArray()` deletes wrapped in `db.transaction()` for atomic removal - no stale data remains. Events trigger comprehensive logging with request IDs. Always returns 200 OK to prevent retries, even for auth failures. Webhook URL: `https://www.flint-investing.com/api/webhooks/snaptrade`, configured via SNAPTRADE_WEBHOOK_SECRET environment variable. Provides instant updates for account changes, eliminating need for aggressive polling of positions/balances endpoints.
