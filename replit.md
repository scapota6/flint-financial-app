...
- **SnapTrade Storage Architecture**: SnapTrade user credentials are stored in file-based storage (`data/snaptrade-users.json`) via `server/store/snapUsers.ts`. The file is keyed by Flint user ID (`req.user.claims.sub`). All SnapTrade routes in `server/routes/snaptrade.ts` use `getSnapUser()` to access file storage. Critical: SnapTrade routes must be mounted AFTER authentication initialization in `server/index.ts` to ensure `req.user.claims.sub` is available. The database table `snaptrade_users` exists but is not actively used; file storage is the source of truth.
