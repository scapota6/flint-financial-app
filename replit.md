...
- **Subscription System**: Three-tier model (Free, Basic, Pro) integrated with Whop.com for payment processing and feature gating, with calendar-aware next billing date calculations. **Checkout Flow**: Embedded checkout modal using Whop's vanilla JS loader (`https://js.whop.com/static/checkout/loader.js` loaded in index.html). Modal uses data attributes (`data-whop-checkout-plan-id`, `data-whop-checkout-skip-redirect`, `data-whop-checkout-theme`) to embed checkout iframe. Server-side endpoint `POST /api/whop/create-checkout` returns planId which is injected into checkout container div. Modal stays on Flint site (no new tab/redirect) and listens for `whop-checkout-complete` event. On successful payment, frontend calls `POST /api/whop/activate-subscription` with planId and receiptId to update user subscription status in database. Backend uses plan IDs (plan_n0ViyXRIGl5q4, plan_eK9OYHEBv6qac, plan_McZjMF8v2N655, plan_1ZXWf1eqiwjcT) configured in `server/lib/whop-config.ts`. After activation, user redirects to dashboard. Webhooks handle subscription lifecycle events, user creation, and email delivery. Free: 4 connections, money in/out flow, dashboard, transaction history. Basic: unlimited connections, all Free features + recurring subscriptions, credit card management, stock charts (coming soon). Pro: all Basic features + trading & transfers (coming soon).
