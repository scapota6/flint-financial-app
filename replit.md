...
- **Subscription System**: Three-tier model (Free, Basic, Pro) integrated with Whop.com for payment processing and feature gating, with calendar-aware next billing date calculations. **Checkout Flow**: Embedded checkout modal using `@whop/checkout/react` WhopCheckoutEmbed component. Server-side endpoint `POST /api/whop/create-checkout` returns Whop product ID which is passed to the embedded checkout. Modal is dismissable via ESC key and outside clicks. Backend uses new product IDs (prod_kNrAAxyPkLfuN, prod_DOqriggmULL56, prod_3MKtU8x9IHAEw, prod_OgQRS19ljYw6N) configured in `server/lib/whop-config.ts`. Component configured with `skipRedirect: true` to keep users on Flint site after payment, and `onComplete` callback handles successful purchases. Webhooks handle subscription activation, user creation, and email delivery. Free: 4 connections, money in/out flow, dashboard, transaction history. Basic: unlimited connections, all Free features + recurring subscriptions, credit card management, stock charts (coming soon). Pro: all Basic features + trading & transfers (coming soon).
