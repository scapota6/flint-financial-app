...
- **Subscription System**: Three-tier model (Free, Basic, Pro) integrated with Whop.com for payment processing and feature gating, with calendar-aware next billing date calculations. **Checkout Flow**: Server-side endpoint `POST /api/whop/create-checkout` returns Whop product URL (`purchaseUrl`) which opens in a centered popup window (600x800px). Popup approach chosen over embedded iframe due to browser cross-origin security restrictions that prevent communication between Whop's checkout and their payment processor (BasisTheory). The popup window keeps the main Flint page visible in the background, detects popup blockers, monitors window closure, and shows user feedback via toast notifications. Attempted Whop's official `@whop/checkout` React embed component but encountered fatal postMessage errors blocking payment form interaction - browser security policies prevent embedded third-party payment processors regardless of official SDK support. Webhooks handle subscription activation, user creation, and email delivery. Free: 4 connections, money in/out flow, dashboard, transaction history. Basic: unlimited connections, all Free features + recurring subscriptions, credit card management, stock charts (coming soon). Pro: all Basic features + trading & transfers (coming soon).
