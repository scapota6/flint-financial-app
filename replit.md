...
- **Email Service**: Resend-based email system using Replit connector for secure API key management. Configured to send from `updates@flint-investing.com` with replies directed to `support@flint-investing.com`. Includes automated email logging to database. **Note**: Custom domain requires verification in Resend dashboard (https://resend.com/domains) - add and verify `flint-investing.com` before production use. Test endpoint available at `POST /api/admin/test-email`.
