...
### Key Components
- **Public Registration System (Nov 2025)**: Production-ready user registration via `/api/auth/public-register` accepting firstName, email, and password. Features Argon2id password hashing, account enumeration prevention (returns generic success message for duplicate emails), password strength validation (8+ chars, uppercase, lowercase, number, special char), IP-based rate limiting (3 attempts per 15 min), CSRF bypass for public access, and automatic waitlist position assignment (3,285/10,000). Sets emailVerified=false with TODO for proper email verification flow. Integrated with landing page signup form with loading states and redirect to /login on success.
